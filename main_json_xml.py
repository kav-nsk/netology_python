...
def get_word_over_six_let(incomingText):
    ...
    for string in incomTextList:    # Удаляем цифры.
        if string.isdigit():
            incomTextList.remove(string)
    for word in incomTextList:      # Ищем слова длиннее 6 букв.
        if len(word) > 6:
            outputSet.add(word)     # Добавляем их в результирующее множество.
    return outputSet
...
def print_result_top10(resultOverlapsDict, title):
    ...
    print('++++ 10 групп слов длиннее шести букв наиболее часто встречающихся в новостных лентах формата %s ++++' % title)
    for i in keyList:
        print('Слова повторяющиеся %d раз: ' % i, end='')
        for j in resultOverlapsDict[i]:
            print(j, ' ', sep='', end='')
        print('\n')
    return True
...
for i in range(lenJsonText):
    allTextDict[i] = jsonText['rss']['channel']['items'][i]['description']
...
print_result_top10(resultOverlapsDict, '.json')
    ...
    # Обработка XML файла ------------------------------------------------------------------------------------
tree = ElementTree()
xmlTree = tree.parse('newsafr.xml')
